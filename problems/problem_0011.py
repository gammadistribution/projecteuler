...
def get_matrix(path):
    """Read in grid from given path. Each row in file represents row of matrix
    and each space designates a new column. Convert each entry to integer and
    convert list of rows to numpy.matrix object. Returns numpy.matrix object
    corresponding to entries found in path.
    """
    with open(path) as f:
        lines = f.readlines()

    rows = [[int(entry) for entry in line.split(' ')] for line in lines]
